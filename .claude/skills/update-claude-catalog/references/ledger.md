...
| 官方模型（`custom.toml`） | platform.claude.com 模型总览 + 二进制 | 2026-09-21 |
...
| `openrouter` / `huggingface` | 各自 live 目录 | 2026-09-21 |
| `fireworks` / `vercel` / `opencode` / `opencode-go` | pi 目录里的 anthropic-messages 模型 | 2026-09-21 |
| `xai` / `mimo` / `mimo-plan` | 厂商页只渲染出部分 id，没能完整重读；pi 目录无新增 | 2026-09-14 |
| `siliconflow` | siliconflow.cn/models（需登录，没重读） | 2026-09-14 |
| 各渠道 `/v1/messages` 端点 | `scripts/probe-endpoint.mjs` | 2026-09-14 |
| pi 目录快照（`pi-models.json`） | pi.dev/api/models（Last-Modified 2026-09-18）+ npm `@earendil-works/pi-ai` 0.86.1 | 2026-09-21 |
